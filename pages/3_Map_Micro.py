...
with c2: kpi_card("Geo coverage", f"{view[['latitude','longitude']].dropna().shape[0]:,}" if "latitude" in view.columns and "longitude" in view.columns else "n/a", "With lat/lon")
with c3: kpi_card("Price/sqm coverage", f"{view['price_per_sqm'].notna().mean():.0%}" if "price_per_sqm" in view.columns else "n/a", "Non-null share")
with c4: kpi_card("DOM coverage", f"{view['days_on_market'].notna().mean():.0%}" if "days_on_market" in view.columns else "n/a", "Non-null share")
